...
use softfloat_sys_riscv::float16_t;
...
impl Float for F16 {
    ...
    fn add<T: Borrow<Self>>(&self, x: T, rnd: RoundingMode) -> Self {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::f16_add(self.0, x.borrow().0) };
        Self(ret)
    }

    fn sub<T: Borrow<Self>>(&self, x: T, rnd: RoundingMode) -> Self {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::f16_sub(self.0, x.borrow().0) };
        Self(ret)
    }

    fn mul<T: Borrow<Self>>(&self, x: T, rnd: RoundingMode) -> Self {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::f16_mul(self.0, x.borrow().0) };
        Self(ret)
    }

    fn fused_mul_add<T: Borrow<Self>>(&self, x: T, y: T, rnd: RoundingMode) -> Self {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::f16_mulAdd(self.0, x.borrow().0, y.borrow().0) };
        Self(ret)
    }

    fn div<T: Borrow<Self>>(&self, x: T, rnd: RoundingMode) -> Self {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::f16_div(self.0, x.borrow().0) };
        Self(ret)
    }

    fn rem<T: Borrow<Self>>(&self, x: T, rnd: RoundingMode) -> Self {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::f16_rem(self.0, x.borrow().0) };
        Self(ret)
    }

    fn sqrt(&self, rnd: RoundingMode) -> Self {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::f16_sqrt(self.0) };
        Self(ret)
    }

    fn compare<T: Borrow<Self>>(&self, x: T) -> Option<Ordering> {
        let eq = unsafe { softfloat_sys_riscv::f16_eq(self.0, x.borrow().0) };
        let lt = unsafe { softfloat_sys_riscv::f16_lt(self.0, x.borrow().0) };
    ...
    fn from_u32(x: u32, rnd: RoundingMode) -> Self {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::ui32_to_f16(x) };
        Self(ret)
    }

    fn from_u64(x: u64, rnd: RoundingMode) -> Self {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::ui64_to_f16(x) };
        Self(ret)
    }

    fn from_i32(x: i32, rnd: RoundingMode) -> Self {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::i32_to_f16(x) };
        Self(ret)
    }

    fn from_i64(x: i64, rnd: RoundingMode) -> Self {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::i64_to_f16(x) };
        Self(ret)
    }

    fn to_u32(&self, rnd: RoundingMode) -> u32 {
        let ret = unsafe { softfloat_sys_riscv::f16_to_ui32(self.0, rnd.to_softfloat(), false) };
        ret as u32
    }

    fn to_u64(&self, rnd: RoundingMode) -> u64 {
        let ret = unsafe { softfloat_sys_riscv::f16_to_ui64(self.0, rnd.to_softfloat(), false) };
        ret
    }

    fn to_i32(&self, rnd: RoundingMode) -> i32 {
        let ret = unsafe { softfloat_sys_riscv::f16_to_i32(self.0, rnd.to_softfloat(), false) };
        ret as i32
    }

    fn to_i64(&self, rnd: RoundingMode) -> i64 {
        let ret = unsafe { softfloat_sys_riscv::f16_to_i64(self.0, rnd.to_softfloat(), false) };
        ret
    }
    ...
    fn to_f32(&self, rnd: RoundingMode) -> F32 {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::f16_to_f32(self.0) };
        F32::from_bits(ret.v)
    }

    fn to_f64(&self, rnd: RoundingMode) -> F64 {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::f16_to_f64(self.0) };
        F64::from_bits(ret.v)
    }

    fn to_f128(&self, rnd: RoundingMode) -> F128 {
        rnd.set();
        let ret = unsafe { softfloat_sys_riscv::f16_to_f128(self.0) };
        let mut v = 0u128;
        v |= ret.v[0] as u128;
    ...
    fn round_to_integral(&self, rnd: RoundingMode) -> Self {
        let ret = unsafe { softfloat_sys_riscv::f16_roundToInt(self.0, rnd.to_softfloat(), false) };
        Self(ret)
    }
}
